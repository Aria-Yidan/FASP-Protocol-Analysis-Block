...
/*
typedef struct wm_search_output
{
	char str[MAXM];
	struct WM_SEARCH_OUTPUT *next;
}WM_SEARCH_OUTPUT;

WM_SEARCH_OUTPUT *WM_SEARCH_RESULT = NULL;
WM_SEARCH_OUTPUT *WM_SEARCH_RESULT_TEMP = NULL;
*/
...
WM_STRUCT * wmNew()
{
	WM_STRUCT *p=(WM_STRUCT *)vmalloc(sizeof(WM_STRUCT));
	if(!p) return 0;
...
void wmFree(WM_STRUCT *ps) //释放空间函数
{
	...
	{
		if(ps->msPatArray->psPat) vfree(ps->msPatArray->psPat);	//子串不为空，则释放
		vfree(ps->msPatArray );
	}
	if(ps->msNumArray) vfree(ps->msNumArray);
	if(ps->msHash) vfree(ps->msHash);
	if(ps->msPrefix) vfree(ps->msPrefix);
	if(ps->msShift) vfree(ps->msShift);
	vfree(ps);
}
...
int wmAddPattern(WM_STRUCT *ps,unsigned char *q,int m)
{
	...
	p=(WM_PATTERN_STRUCT *)vmalloc(sizeof(WM_PATTERN_STRUCT));
	if(!p) return -1;

	p->psPat=(unsigned char*)vmalloc(m+1); //据子串数组的长度分配空间
...
static void wmPrepHashedPatternGroups(WM_STRUCT *ps)
{
	...
	ps->msHash=(HASH_TYPE*)vmalloc(sizeof(HASH_TYPE)* ps->msNumHashEntries);	//HASH表
	if(!ps->msHash)
	{
...
static void wmPrepShiftTable(WM_STRUCT *ps)
{
	...
	ps->msShift=(unsigned char*)vmalloc(SHIFTTABLESIZE*sizeof(char));
	if(!ps->msShift)
		return;
...
{
	int i;
	ps->msPrefix=(HASH_TYPE*)vmalloc(sizeof(HASH_TYPE)* ps->msNumPatterns);	//分配空间长度为所有子串的个数*
	if(!ps->msPrefix)
	{
	...
	for(;patrn<patrnEnd;patrn++)
	{
		...
		else	//如果后缀哈希值相同，则
		{
			...
			if(*(px-1)=='\0')	//匹配到了结束位置，说明匹配成功
			{
				...
				/*
				WM_SEARCH_OUTPUT *tmp = (WM_SEARCH_OUTPUT *)vmalloc(sizeof(WM_SEARCH_OUTPUT));
				strcpy(tmp->str, (const char *)patrn->psPat);
				tmp->next = NULL;
				WM_SEARCH_RESULT_TEMP->next = tmp;
				WM_SEARCH_RESULT_TEMP = tmp;
				*/
...
int wmPrepPatterns(WM_STRUCT *ps)
{
	...
	ps->msPatArray=(WM_PATTERN_STRUCT*)vmalloc(sizeof(WM_PATTERN_STRUCT)*ps->msNumPatterns);
	if(!ps->msPatArray)
		return -1;

	ps->msNumArray=(unsigned short*)vmalloc(sizeof(short)*ps->msNumPatterns);
	if(!ps->msNumArray)
		return -1;
...
void InitFilePattern(WM_STRUCT *p, char pattern[][MAXM])
{
	int n = 0, length = 0;
	// p = wmNew();	//创建模式串集
