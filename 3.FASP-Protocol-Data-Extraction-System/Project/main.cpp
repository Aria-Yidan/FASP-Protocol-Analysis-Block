...
BOOL __stdcall DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:	// 初始化程序窗口
	{
						  ...
						  ::SendMessage(hWndStatus, SB_SETTEXT, 1, (long)" \t");
...
void UpdateProcess(HWND hWndList)
{
	...
	item.pszText = (LPTSTR)TEXT("FASP Transfer Begin Request");	// 设置文本
	...
	wsprintf(relatedINF, "%d", FASP_num);
	...
	item.mask = LVIF_TEXT;			// 指定pszText域有效
	item.pszText = (LPTSTR)TEXT("FASP Data Block Numbers");	// 设置文本
	...
	wsprintf(relatedINF, "%d", FASP_BLOCKNUM);
	lvi.pszText = (LPTSTR)relatedINF;	// 要设置的文本
	::SendMessage(hWndList, LVM_SETITEMTEXT, nItem, (LPARAM)&lvi);
...
int BeginGetPacket(int count)
{
	...
	sprintf(filename, "FASP_%d", timeBegin);
	fp_FASP = fopen(filename, "ab+");
	...
	fprintf(fp, "\tICMP: %d packet\r\n", ICMP_num);
	fprintf(fp, "FASP Transfer Begin Request: %d packet\r\n", FASP_num); 
	fprintf(fp, "FASP Data Block Numbers: %d packet\r\n", FASP_BLOCKNUM);
