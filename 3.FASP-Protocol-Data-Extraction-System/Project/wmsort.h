...
extern int nline = 1;
extern int nfound = 0;
#define MAXN 10001 // 文本串的最大长度MAXN - 1
#define MAXM 51// 模式串最大长度为MAXM - 1
...
struct WM_SEARCH_OUTPUT
{
	char str[MAXM];
	struct WM_SEARCH_OUTPUT *next;
};

WM_SEARCH_OUTPUT *WM_SEARCH_RESULT = new WM_SEARCH_OUTPUT;	// WM算法匹配结果
WM_SEARCH_OUTPUT *WM_SEARCH_RESULT_TEMP = WM_SEARCH_RESULT;	// WM算法匹配结果temp
...
目的：后缀哈希值相同，比较前缀以及整个字符串匹配, (增加：记录匹配结果到WM_SEARCH_RESULT)
参数：
ps => 模式串集
lindex =>
Tx => 要进行匹配的字符串序列
	...
	for (; patrn<patrnEnd; patrn++)
	{
		...
		else	//如果后缀哈希值相同，则
		{
			...
			if (*(px - 1) == '\0')	//匹配到了结束位置，说明匹配成功
			{
				...
				WM_SEARCH_OUTPUT *temp = new WM_SEARCH_OUTPUT;
				strcpy(temp->str, (const char*)patrn->psPat);
				temp->next = NULL;
				WM_SEARCH_RESULT_TEMP->next = temp;
				WM_SEARCH_RESULT_TEMP = temp;
